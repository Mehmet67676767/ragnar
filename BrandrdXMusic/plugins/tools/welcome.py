...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        temp.MELCOW[f"welcome-{member.chat.id}"] = await app.send_photo(
            ...
            photo=welcomeimg,
            caption=f"""
👋 {member.chat.title} grubuna hoş geldin!
➖➖➖➖➖➖➖➖➖➖➖
๏ İSİM ➠ {user.mention}
๏ ID ➠ {user.id}
๏ KULLANICI ADI ➠ @{user.username}
๏ HAZIRLAYAN ➠ [Branded 🇽 King 📿](https://t.me/BRANDEDKING8)
➖➖➖➖➖➖➖➖➖➖➖
""",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("⦿ Beni Grubuna Ekle ⦿", url=f"https://t.me/Sustumm_music_bot?startgroup=true")]]
            )
        )
    except Exception as e:
        LOGGER.error(e)
...
📢 YENİ GRUP EKLENDİ
➖➖➖➖➖➖➖➖➖➖➖
📌 İSİM: {message.chat.title}
🆔 ID: {message.chat.id}
👤 KULLANICI ADI: @{message.chat.username}
➖➖➖➖➖➖➖➖➖➖➖
""")